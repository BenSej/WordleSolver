...
if __name__ == '__main__':
    ...
    def play():
        while len(solutionSet) > 1:
            word = str(input("Input: "))
            colors = input("Colors: ")
            colorArray = []
            for char in colors:
                colorArray.append(int(char))
            next, nextAverage = nextWord(word, colorArray, solutionSet)
            ...
            if len(solutionSet) < 20:
                ...
                for w in solutionSet:
                    tempSet = solutionSet.copy()
                    average = averageEliminations(w, tempSet)
                    if average > bestWordAverage:
                        bestWord = w
                        bestWordAverage = average
                    averages[w] = average

                print("Average Eliminations Per Solution: " + str(averages))
                print("Best Word In Solution Set: " + str(bestWord).upper() + " (" + str(bestWordAverage) + ")")
                print("Otherwise: " + next.upper() + " (" + str(nextAverage) + ")")
            else:
            ...
            while len(solutionSet) > 1:
                ...
                for j in range(5):
                    if word[j] == w[j]:
                        ...
                        colors[j] = 1
                    elif word[j] not in w:
                        colors[j] = 0
                next = nextWord(word, colors, copy)
                word = next
                if len(copy) == 1:
                    break
